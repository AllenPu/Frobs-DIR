...
def match_A_in_B(A: torch.Tensor, B: list):
    ...
    mask = torch.isin(a, b)                 # (N,)
    idx_in_a = torch.nonzero(mask).flatten()  # (K,)
    vals = a[idx_in_a]                      # values that are in both
...
###################################################
def cal_per_label_frobs_mae(model_regression, train_loader, test_loader, model_name=''):
